...
- **Hermes/OpenRouter usage in USAGE & LIMITS**, for whichever FLEET hosts run Hermes. Reads Hermes's own local billing ledger (`~/.hermes/state.db`, one bounded `sqlite3 -readonly -json` call) instead of calling OpenRouter directly, so there is no second API key to hold anywhere on the machine. `pricing.json`'s pinned LiteLLM snapshot has no entries for most OpenRouter model ids, so this is the one usage row whose dollar estimate does not come from that table — Hermes already resolves a live OpenRouter rate per row (`cost_source: provider_models_api`) that is at least as current as a pinned snapshot would be, and the card's status line says the number is Hermes's own estimate, not this project's.
