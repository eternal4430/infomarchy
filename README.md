...
When a host is running Hermes, USAGE & LIMITS gains a Hermes row too, sourced from Hermes's own local billing ledger (`~/.hermes/state.db`, read with `sqlite3 -readonly`) rather than an OpenRouter API call — no second key to manage, and the dollar figure is exactly what Hermes itself resolved from OpenRouter's live pricing at the time, which is why it's labelled *estimated by Hermes*, not sourced from this project's own `pricing.json`.
...
FLEET dials nothing unless `INFOMARCHY_FLEET_HOSTS` names a host, and reads only that host — never a scan, never a discovery step. Each host gets one `ssh -o BatchMode=yes -o ConnectTimeout=4 <host> <fixed command>` call per refresh; an unknown host key or a password prompt fails the probe instead of hanging or falling back to interactive auth, and the remote command is a fixed string, never built from `INFOMARCHY_FLEET_HOSTS` beyond the host argument itself, so there is nothing in that variable for an entry to inject into. Presence detection runs `ps -eo pid=,args=`, bounded on both ends; usage reads Hermes's own `~/.hermes/state.db` with `sqlite3 -readonly`, also bounded. No OpenRouter or other third-party API is called directly, and no API key is stored anywhere for this feature. `INFOMARCHY_SKIP_FLEET=1` disables both probes.
